    ...
    public void serialize(long src, long dst, int vertexCount) {
        ...
        for (int i = 0; i < quadCount; i++) {
            ...
            int tangent = NormalHelper.computeTangent(
                ...
                MemoryUtil.memGetFloat(src + 2 * EntityVertex.STRIDE),
                MemoryUtil.memGetFloat(src + 4 + 2 * EntityVertex.STRIDE),
                MemoryUtil.memGetFloat(src + 8 + 2 * EntityVertex.STRIDE),
                MemoryUtil.memGetFloat(src + 16 + 2 * EntityVertex.STRIDE),
                MemoryUtil.memGetFloat(src + 20 + 2 * EntityVertex.STRIDE)
            );
